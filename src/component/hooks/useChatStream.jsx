...
const useChatStream = ({ issuer, appId, roleArn, region, email }) => {
  ...
  const fetchIdToken = () => {
    const CLIENT_ID = 'oidc-tvm-867344447185';
    const CLIENT_SECRET = '7c50f35f3288f04bca7108f6';
    const authHeader = btoa(`${CLIENT_ID}:${CLIENT_SECRET}`);
  
    return fetch(`${issuer.replace(/\/$/, "")}/token`, {
      method: 'POST',
      headers: {
  ...
  const handleSocketCloseOrErr = async(e) => {
    ...
    // Internal server error
      ...
      let exceptionMessage = errorMap.error['websocket-conn-err'];
      let exceptionType = 'UnknownException';
      let errorMessage = "";
      if (e.data) {
        const decodedEvent = await decodeEventStream(e.data);
        exceptionMessage = decodedEvent.data;
        exceptionType = decodedEvent.exceptionType;
      }
